...
################################################################################################################################################################################################################# #
# insert 2 TOC stock barplots report ####
############################################################################################################################################################################################################### #

# for report ####
{
  #svg(paste0(root_dir,"/GitHub/R_main/R_main/temp/BDF_TOC_frisch_full.svg"),width = 8,height = 8)
  plot(ggplot(
  )+
    #manual gridlines
    #   geom_hline(yintercept = seq(.6,2.8,.1),col="grey15",linewidth=.025)+
...
# for report ####
{
 # svg(paste0(root_dir,"/GitHub/R_main/R_main/temp/BDF_TOC_Bilanz_frisch_full.svg"),width = 8,height = 8)
  plot(ggplot(
  )+
    #manual gridlines
    #   geom_hline(yintercept = seq(.6,2.8,.1),col="grey15",linewidth=.025)+
    ...
    geom_line(data=(BDF_frisch$TRD%>%filter(sample_id%>%substr(2,2)%in%c("R","Q","P")&Tiefe_von>=0)%>%left_join(BDF_frisch$soliTOC,by=c("sample_id"="Name"))), 
              aes(
                x=(Tiefe_bis+Tiefe_von)/2,
                y=Lutro_density_all*TOC,   # same as weighed for TOC determination
                linetype=Typ,
                alpha=Typ,
                col=Typ,
                group=paste(BDF,Position,Typ)),
...
{
  #svg(paste0(root_dir,"/GitHub/R_main/R_main/temp/TOC_budget_full.svg"),width = 8,height = 4)
  plot(
...
#barplot aggregated with errorbar 30####
{
  ...
  #svg(paste0(root_dir,"/GitHub/R_main/R_main/temp/TOC_budget_full.svg"),width = 10,height = 4)
  plot( 
    ggarrange(
      (
        BDF_frisch$TRD%>%filter(sample_id%>%substr(2,2)%in%c("R","Q")&Tiefe_von>=0)%>%
...
#barplot aggregated with errorbar 50####
{
 ...
 # svg(paste0(root_dir,"/GitHub/R_main/R_main/temp/TOC_budget_full_50.svg"),width = 10,height = 4)
  plot(
    ggarrange(
      (
        BDF_frisch$TRD%>%filter(sample_id%>%substr(2,2)%in%c("R","Q")&Tiefe_von>=0)%>%
...
Lagerungsdichte_Referenz_BUDGET=read_excel("~/GitHub/R_main/data/soil_physics/Lagerungsdichten_Referenz.xlsx",sheet = "Tabelle2", na = "NA")
...
allBDF_OSSL_data%>%
  mutate(dB=if_else(Campaign%>%str_detect("field"),`dB_105 [g/cm3]`,`dB [g/cm3]`))%>%
  filter(Device!="Profilspaten")%>%
ggplot(aes(x=dB,y=bd_usda.a4_g.cm3_pred))+
  geom_abline(slope = 1)+
  geom_point(aes(col=str_split_fixed(Campaign,"_",2)[,2],shape=str_split_fixed(Campaign,"_",2)[,2]))+
  theme_pubr()+
  scale_shape_discrete("Bulk density [g/cm³]",
                       breaks=c("archive","field"),
                       labels=c("Archive","Field"))+
  scale_color_manual("Bulk density [g/cm³]",
                     breaks=c("archive","field"),
                     labels=c("Archive","Field"),
                     values = colorblind_safe_colors()[c(4,7)])+
  xlab("Observed")+
  ylab("Predicted")->dBOSSL


ggsave(dBOSSL,filename = "dB_OSSL_obspred.png",
       path = "C:/Users/adam/Desktop/UNI/PhD/DISS/plots/",
       height=4,width=4,device="png")

bind_rows(
tibble(set="all",allBDF_OSSL_data%>%
  mutate(dB=if_else(Campaign%>%str_detect("field"),`dB_105 [g/cm3]`,`dB [g/cm3]`))%>%
  filter(Device!="Profilspaten")%>%
evaluate_model_adjusted(obs="dB",pred=bd_usda.a4_g.cm3_pred)),

tibble(set="archive",allBDF_OSSL_data%>%
         mutate(dB=if_else(Campaign%>%str_detect("field"),`dB_105 [g/cm3]`,`dB [g/cm3]`))%>%
         filter(Device!="Profilspaten"&!Campaign%>%str_detect("field"))%>%
         evaluate_model_adjusted(obs="dB",pred=bd_usda.a4_g.cm3_pred)),

tibble(set="field",allBDF_OSSL_data%>%
         mutate(dB=if_else(Campaign%>%str_detect("field"),`dB_105 [g/cm3]`,`dB [g/cm3]`))%>%
         filter(Device!="Profilspaten"&Campaign%>%str_detect("field"))%>%
         evaluate_model_adjusted(obs="dB",pred=bd_usda.a4_g.cm3_pred)),
...
tmp%>%filter(Device!="Profilspaten"&u3<=.5)%>%
  ...
  mutate(name=factor(name,levels=c("obs","predOSSL_dB105_TOC","pred_dB105_TOC","pred_dB105_pred_TOC")))%>%
  ggplot()+
